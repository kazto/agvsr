...
### Phase 20 — Web Gateway Phase 4: Web Push Notifications ✅

| File / area                             | What it does                                                                                                                                                                                                                                                                                                      |
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/hooks.ts`                          | Adds `PushPayload { job_id, status }`, `PushNotifier` type, and `noopPushNotifier` default.                                                                                                                                                                                                                       |
| `src/web/auth-store.ts`                 | Adds `web_vapid_keys` (singleton row: P-256 ECDSA public+private base64url) and `web_push_subscriptions` (endpoint PK, p256dh, auth) tables. Adds `getOrCreateVapidKeys`, `getVapidPublicKey`, `addPushSubscription`, `listPushSubscriptions`, `removePushSubscription`.                                         |
| `src/web/push.ts`                       | Full RFC 8291 aes128gcm Web Push encryption (ECDH P-256 + HKDF-SHA256 + AES-128-GCM) and RFC 8292 VAPID JWT signing — all via Bun built-in WebCrypto, no new runtime dependencies. Exports `sendPush`, `createPushNotifier` (fire-and-forget with lazy store + dead-endpoint pruning), and `deriveEncryptionKeys` for test use. |
| `src/web/security.ts`                   | Adds `worker-src 'self'` to CSP to allow service worker registration.                                                                                                                                                                                                                                             |
| `src/web/routes.ts`                     | Adds `GET /sw.js` (with `Service-Worker-Allowed: /`), `GET /api/push/config` (VAPID public key), `POST /api/push/subscribe`, `POST /api/push/unsubscribe`. All mutation routes enforce session + Origin + CSRF.                                                                                                   |
| `src/web/server.ts`                     | Loads `src/web/client/sw.ts` as a client asset; calls `getOrCreateVapidKeys()` at startup.                                                                                                                                                                                                                        |
| `src/web/client/sw.ts`                  | Service worker: `push` event parses `{job_id, status}` and calls `showNotification`; `notificationclick` focuses/navigates existing window or opens `/#/jobs/<id>`.                                                                                                                                              |
| `src/web/client/app.ts`                 | Adds push toggle button to the banner. `initPush()` registers `/sw.js`, checks existing subscription, and shows the toggle. Subscribe/unsubscribe path uses `/api/push/config` for the VAPID key and POSTs to the subscription routes. All DOM via `textContent`/DOM APIs.                                       |
| `src/web/client/styles.css`             | Adds `.push-toggle`, `.push-toggle:hover`, `.push-toggle[data-subscribed="1"]`, `.push-toggle--hidden` classes.                                                                                                                                                                                                   |
| `src/daemon/daemon.ts`                  | Accepts injected `pushNotifier?: PushNotifier`; calls it in the `hook()` helper at `on_job_done` (status=done), `on_job_stalled` (status=stalled), `on_supervisor_message` (status=attention), `on_job_failed` (status from `store.getJob()` for correct interrupted/failed/stopped distinction).                 |
| `src/cli/agvsr.ts`                      | Wires `createPushNotifier(storeFile)` into `startDaemon` at daemon-start time.                                                                                                                                                                                                                                    |
| `test/web-push.test.ts`                 | 21 tests: VAPID key idempotency, P-256 key format (65-byte uncompressed), subscription CRUD/upsert, push route security (401/403/400), `/sw.js` headers, VAPID smoke (key shape), full RFC 8291 encrypt/decrypt round-trip with no mocked crypto, `Bun.serve` fake push endpoint verifying aes128gcm + VAPID headers, 404/410 endpoint pruning, `createPushNotifier` pruning integration. |
| `test/hooks.test.ts`                    | 4 new tests: pushNotifier spy for `done`, `failed`, `interrupted`, `attention` statuses wired from each daemon hook point.                                                                                                                                                                                         |
| `test/web-auth.test.ts`                 | Updated table-list assertion to include `web_push_subscriptions` and `web_vapid_keys`.                                                                                                                                                                                                                             |

**Real-crypto vs spy/mock coverage distinction:**
- Real crypto (no mocking): VAPID P-256 key generation, full RFC 8291 encrypt/decrypt round-trip, `Bun.serve` fake endpoint header/body parsing, 404/410 endpoint pruning.
- Spy/wiring: pushNotifier payload correctness for all four daemon hook points, push route auth/CSRF/Origin, subscription CRUD.
...
Merged into main alongside Phase 19 (Turn-Failure Diagnostics): **303 passing tests, 0 failing**; `bun run typecheck`, `bunx oxlint src test`, and `bunx oxfmt --check src test` all pass.

### Phase 21 — Web Gateway Phase 5: daemon lifecycle push (`job.update`) ✅

| File / area                             | What it does                                                                                                                                                                                                                                                                                                      |
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/protocol.ts`                       | `PushFrame` extended from single interface to discriminated union (`msg.new` \| `job.update`). `JobUpdate { job_id, status, updated_at }` payload type added. `job.watch` (params-less) added to `Request` union.                                                                                                  |
| `src/daemon/daemon.ts`                  | `jobWatchers: Set<watcher>` (global, mirrors `msgWatchers` pattern). `emitJobUpdate(jobId, status)` fires `job.update` frames to all live watchers with prune-on-dead-connection. `setStatus(jobId, status)` wrapper replaces all 11 `store.setJobStatus` call sites (single choke-point). `emitJobUpdate(job.id, "running")` fires after successful `job.create`. `job.watch` handler registers global subscribers. |
| `src/web/stream.ts`                     | `StreamSocketData` exported as discriminated union `{ kind: "job"; jobId } \| { kind: "jobs" }`. `job.watch` sent once at bridge startup. `jobsSubscribers` set for global clients. `deliverJobUpdate` fans out `job.update` to all global subscribers. `onPush` dispatches by `frame.event`. `open`/`close` handlers branch on `kind`. |
| `src/web/server.ts`                     | `/api/jobs/stream` upgrade path added before per-job parser; same Host/Origin/session checks enforced. Upgrades with `{ kind: "jobs" }` data. Per-job path unchanged (`{ kind: "job", jobId }`).                                                                                                                 |
| `src/web/client/app.ts`                 | Primary 2-second `startPolling` loop removed. Initial load (once) does `refreshSession` + `refreshJobs` + `initPush` + `openJobsStream`. Global `/api/jobs/stream` WebSocket drives debounced (250ms) `refreshJobs` on `job.update`; refreshes selected job detail if relevant. Reconnects with backoff; refreshes jobs on reconnect. 30-second fallback poll retained for session drift and WS gap recovery. `FALLBACK_POLL_MS = 30000` exported. |
| `test/protocol.test.ts`                 | New: `PushFrame` discriminated union round-trip for both variants; `job.watch` request encoding.                                                                                                                                                                                                                   |
| `test/job-update.test.ts`               | New: real daemon + real IPC — `job.watch` response, `job.create` running emit, `job.complete/stop/kill/fail` terminal emits each exactly once, payload fields, pre-subscription isolation, multi-subscriber fanout.                                                                                                |
| `test/web-ws.test.ts`                   | Extended: `/api/jobs/stream` delivery to authenticated client, two-client fanout, per-job stream isolation (no `job.update`), 401 and 403 rejection paths.                                                                                                                                                        |
| `test/ipc.test.ts`, `test/watch.test.ts` | Narrowed `PushFrame` union property accesses for TypeScript correctness (behavior unchanged).                                                                                                                                                                                                                     |
