...
- **基準値の採取**: **元チェックアウトで**同じコマンドを実行して件数を採る。
  元チェックアウトには `.env` があるので、これが「本来の件数」になる。
  結果は `(repoRoot, HEAD, command)` でキャッシュして再利用する。
  **採取タイミングは `job.create` ではなくゲート時に遅延させた(実装時の変更)。**
  `job.create` で走らせるとジョブ作成が数分ブロックされ、`agvsr job` の
  前景ブロック問題(報告 7)を悪化させる。遅延させても「元チェックアウトで測る」
  という性質は変わらず、キャッシュにより 2 回目以降のジョブは追加コストなしになる。
