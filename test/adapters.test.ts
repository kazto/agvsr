...
describe("claude driver", () => {
  it("builds a new-turn spawn (no --resume)", () => {
    ...
    expect(s.args).toEqual(expect.arrayContaining(["--output-format", "stream-json", "--model", "the-model"]));
  ...
  it("parses init/assistant/result into events", () => {
    ...
    const events = drain(p, [
      ...
      JSON.stringify({
        type: "assistant",
        message: { content: [{ type: "text", text: "hi " }, { type: "tool_use", name: "agvsr_send", input: { to: "supervisor" } }] },
      }),
