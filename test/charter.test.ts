...
describe("fillScaffold", () => {
  const scaffold = "role={{role}} job={{job_id}} cwd={{cwd}} targets={{allowed_targets}}\n[{{completion_tools}}]";
  ...
  it("substitutes placeholders", () => {
    ...
    expect(out).toContain("role=qa job=J1 cwd=/w targets=supervisor");
  });
...
describe("composeCharter (bundled charters)", () => {
  it("composes scaffold + role charter with context filled", () => {
    const prompt = composeCharter(team, "qa", { jobId: "JOB-7", cwd: "/work/repo" });
  ...
  it("gives the supervisor completion tools and worker targets", () => {
    const prompt = composeCharter(team, "supervisor", { jobId: "JOB-7", cwd: "/work/repo" });
